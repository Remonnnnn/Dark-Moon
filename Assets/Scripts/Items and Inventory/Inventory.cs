...
public class Inventory : MonoBehaviour,ISaveManager
{
    ...
    public List<ItemData> startingItems;//起始装备
    ...
    {
        if(loadedEquipment.Count>0)//加载正在装备的物品
        {
    ...
    public void EquipItem(ItemData _item)
    {
        ...
        foreach(KeyValuePair<ItemData_Equipment,InventoryItem>item in equipmentDictionary)//寻找是否已经装备与待装备相同类型的装备
        {
        ...
        if(oldEquipment!=null)//已经装备则替换
        {
            UnequiItem(oldEquipment);
            AddItem(oldEquipment);//将原装备放回仓库
        }

        equipment.Add(newItem);//装备新装备
        equipmentDictionary.Add(newEquipment, newItem);
        newEquipment.AddModifiers();

        RemoveItem(_item);//从仓库中移除
    ...
    private void UpdateSlotUI()
    {
        ...
        {
            foreach (KeyValuePair<ItemData_Equipment, InventoryItem> item in equipmentDictionary)//更新装备栏UI
            {
    ...
    {
        for (int i = 0; i < statSlot.Length; i++)//更新状态信息
        {
    ...
    {
        if (_item.itemType == ItemType.Equipment && CanAdditem())//如果添加的是装备
        {
            AddToInventory(_item);//添加到装备库存
        }
    ...
    private void AddToInventory(ItemData _item)//添加装备
    {
    ...
    private void AddToStash(ItemData _item)//添加材料
    {
    ...
    {
        if (inventoryDictionary.TryGetValue(_item, out InventoryItem value))//移除装备
        {
        ...
        else if(stashDictionary.TryGetValue(_item,out InventoryItem stashValue))//移除材料
        { 
    ...
    public ItemData_Equipment GetEquipment(EquipmentType _type)//得到当前type类型的装备
    {
